...
class TPSLinearInterp():
    ...
    def __init__(self, fun, n, xmin, xmax, ymin, ymax, dim=1):
        ...
        M = np.zeros((num_coords, num_coords+3)) # each row has <num_coords> kernel evals + 2d position vector + 1
        for j in range(num_coords):
            for i in range(num_coords):
                # populate left <num_coords> x <num_coords> block of M with TPS evaluated at each pair of points
                M[i, j] = tps([XX[i],YY[i]], [XX[j],YY[j]])
        for i in range(num_coords):
            # populate columns <num_coords>+1 and <num_coords>+2 with coordinates of position vectors
            M[i, num_coords] = XX[i]
            M[i, num_coords+1] = YY[i]
        ...
        self.beta = c[num_coords:num_coords+2, :] # 2-by-dim matrix of linear coefficients
        self.beta0 = c[num_coords+2, :] # 1-by-dim vector of constants
    ...
    def eval(self, r, z):
        ...
        for j in range(self.n):
            for i in range(self.n):
                result += self.c[:, i, j] * tps([r, z], [XX[i, j], YY[i, j]])
        if np.all(self.fun(r, z) < 1e7): # test if (r, z) is a position vector in return region
